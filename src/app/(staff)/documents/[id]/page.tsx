...
export default async function DocumentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <div className="mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-lg bg-tide-teal/12 text-tide-teal">
            <FileTypeIcon mimeType={currentVersion?.mime_type} className="size-5" strokeWidth={2} />
          </div>
          <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-[15px]">Current file</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {currentVersion ? (
            <div className="flex items-center gap-3.5 rounded-lg border border-border bg-muted/30 px-4 py-3.5">
              <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-tide-teal/12 text-tide-teal">
                <FileTypeIcon mimeType={currentVersion.mime_type} className="size-[18px]" strokeWidth={2} />
              </div>
              <div className="min-w-0 flex-1">
                <div className="truncate text-[14px] font-medium text-tide-charcoal">{currentVersion.file_name}</div>
            ...
            <EmptyState icon={Upload} title="No file uploaded yet" className="border-none py-6" />
          )}

          {canUpload && (
            <form action={uploadDocumentVersion} className="flex flex-wrap items-end gap-2.5 border-t pt-4">
              ...
              <div className="flex-1 space-y-1.5">
                ...
                <input
                  ...
                  name="file"
                  required
                  className="block w-full text-sm text-tide-charcoal file:mr-3 file:rounded-md file:border-0 file:bg-tide-teal/12 file:px-3 file:py-1.5 file:text-[13px] file:font-medium file:text-tide-teal hover:file:bg-tide-teal/20"
                />
              </div>
        ...
        <CardContent className="px-0 py-1">
          <div className="divide-y">
            {versions?.map((v) => {
              ...
              return (
                <div key={v.id} className="flex items-center gap-3.5 px-4 py-3.5 text-sm">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-tide-charcoal">Version {v.version_number}</span>
                      {v.id === currentVersion?.id && (
                        <span className="rounded-full bg-tide-teal/10 px-2 py-0.5 text-[10.5px] font-semibold text-tide-teal">
                          Current
                        </span>
                      )}
                    </div>
                    <div className="truncate text-[12.5px] text-muted-foreground">
                      ...
                      {versionUploader && ` · ${versionUploader.full_name || versionUploader.email}`}
                      {" · "}
