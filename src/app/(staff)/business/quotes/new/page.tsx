...
export default async function NewQuotePage() {
  ...
  return <div className="mx-auto max-w-3xl"><Link href="/business" className="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-tide-charcoal"><ArrowLeft className="size-4" />Business</Link><PageHeader eyebrow="Commercial" title="New quote" description="Create the quote header first, then build its priced line items." /><Card><CardContent><form action={createQuote} className="grid gap-5 sm:grid-cols-2"><CommercialLinkFields organisations={(organisations ?? []).map((item) => ({ id: item.id, label: `${item.client_reference} · ${item.name}` }))} events={(events ?? []).map((item) => ({ id: item.id, label: `${item.event_reference} · ${item.name}` }))} opportunities={(opportunities ?? []).map((item) => ({ id: item.id, label: `${item.opportunity_reference} · ${item.name}` }))} includeOpportunity /><div className="space-y-1.5 sm:col-span-2"><Label htmlFor="title">Quote title</Label><Input id="title" name="title" required /></div><div className="space-y-1.5"><Label htmlFor="issue_date">Issue date</Label><Input id="issue_date" name="issue_date" type="date" defaultValue={today} /></div><div className="space-y-1.5"><Label htmlFor="valid_until">Valid until</Label><Input id="valid_until" name="valid_until" type="date" /></div><div className="space-y-1.5 sm:col-span-2"><Label htmlFor="notes">Client notes</Label><Textarea id="notes" name="notes" rows={3} /></div><div className="space-y-1.5 sm:col-span-2"><Label htmlFor="terms">Full terms and conditions</Label><Textarea id="terms" name="terms" rows={16} defaultValue={terms?.body ?? ""} /></div><Button type="submit" className="w-fit sm:col-span-2">Create quote</Button></form></CardContent></Card></div>;
}
