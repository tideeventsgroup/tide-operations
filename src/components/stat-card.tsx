...
export function StatCard({
  ...
  return (
    <Link
      href={href}
      className="group flex items-center gap-3 rounded-xl border border-border/70 bg-card px-4 py-3.5 transition-colors duration-150 hover:border-tide-teal/40"
    >
      ...
      <div className="min-w-0">
        <div className="text-2xl leading-none font-semibold tracking-tight text-tide-charcoal tabular-nums">
          {value}
        </div>
        <div className="mt-1 truncate text-[12.5px] leading-tight text-muted-foreground">{label}</div>
      </div>
    </Link>
  );
}
